...
for i in range(len(row)):
   ...
   # current_utc_time = current_local_time
    ...
    if (row[i][4] == '0'):
        print("------------------------------------->")
        scheduled_datetime_object = datetime.datetime.strptime(row[i][2] + " " + row[i][1], '%d-%m-%Y %H:%M')
...
for i in row:
    ...
    #cr = con.execute(" select town from selApp_City where user_id=?", (i[4],))
    #print("user_id: ", user_id)
